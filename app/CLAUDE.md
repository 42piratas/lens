...
| `/api/auth/github/start` | ✅ b02-13 | Sets CSRF `state` cookie **on the redirect response**; redirects to the user-authorization endpoint `login/oauth/authorize` (issues the user-to-server token whether or not the app is already installed — `installations/new` only runs the handshake on a fresh install). |
...
- **Connect flow** (auth concern, lives outside the connector folder): `/api/auth/github/start` sets a CSRF `state` cookie **on the redirect response** (the `next/headers` `cookies()` helper does not reliably attach to a `NextResponse.redirect()`) and redirects to **`github.com/login/oauth/authorize`** (`client_id` + `redirect_uri` + `state`). This is the user-authorization endpoint — it issues the user-to-server token whether or not the app is already installed; `apps/<slug>/installations/new` only runs the handshake on a *fresh* install and otherwise just shows the install-management page (which silently broke connect — fixed post-merge). `/api/auth/github/callback` verifies `state`, exchanges `code` for the token server-side (App client secret, never client-side; sends the matching `redirect_uri`), and persists to `oauth_tokens` (`provider = "github"`, no refresh, no expiry). On any failure it redirects to `/settings?github=error&reason=…` (`no_state_cookie` / `state_mismatch` / `token_<github-error>`) for diagnosability. Env vars: `GITHUB_APP_CLIENT_ID` / `GITHUB_APP_CLIENT_SECRET` (`GITHUB_APP_SLUG` no longer required by start, kept for reference).
